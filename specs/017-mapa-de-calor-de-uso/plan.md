...
|---|---|
| **I. Layering** | `httpapi` decodifica el lote y no decide nada · `app.UsageService` orquesta la tx · `domain` tiene la lista blanca, la ventana anti-rebote y la regla de k-anonimato · `store/db` por sqlc |
...
| **VII. Comentarios** | El porqué de la ventana anti-rebote, del `jsonb` vacío y de la supresión por rol va en el código |
...
Lo único periódico es **borrar lo viejo**, una vez al día, dentro del binario que ya corre. No se
agrega un cron ni un contenedor: la VM tiene 300 MB de RAM libres.
