...
- [X] T037 Verificar la paridad local ↔ desplegado de [quickstart.md](./quickstart.md): **cerrado el 2026-09-08** con `web/e2e/deuda-de-especificaciones.spec.ts` › *001/T037*, que corre contra `app-dev` y comprueba lo que se puede automatizar — la vista previa abre sin una sola petición fallida (si dependiera de algo instalado, ahí aparecería), el logo viaja como data URI (un `src` a la API lo bloquearía la CSP de producción) y el botón de imprimir se alcanza sin desplazarse a 1024×600. **De paso encontró un defecto**: ese botón medía 42 px, por debajo del piso táctil de 44, desde que se escribió — corregido. Lo que NO se automatiza y se verifica a mano: el papel.
