...
- [ ] T016 [US3] `web/src/app/MedidorDeToques.tsx`: el escuchador de `pointerdown`/`pointerup` en el
      documento, con el mapa por `pointerId`, el umbral de 10 px y el filtro de capas.
- [ ] T017 [US3] Darle `role="dialog"` y `aria-modal` a `web/src/features/auth/LockScreen.tsx`, con
      su test.
...
- [ ] T029 [P] Medir el paquete del POS contra T002: no más de +5 kB.
...
- **T017 antes que T016 sea útil**: sin el rol en `LockScreen`, el filtro de capas no lo atrapa.
