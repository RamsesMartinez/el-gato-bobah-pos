...
- [X] T004 Escribir `server/migrations/0068_consola_de_plataforma.sql`: la tabla, el rol, los
...
- [X] T004b Test de la política, **antes** de escribirla: con dos empresas en la base, el rol de
...
- [X] T005 Escribir el `Down`: `alter role gatobobah_platform with nologin` más los revokes de lo
...
- [X] T006 Test del `Down` en el mismo archivo: revertir no deja la tabla, deja el rol **sin poder
      entrar**, y volver a aplicar funciona. Es lo que evita descubrir el problema al revertir.

### Configuración y arranque

- [X] T007 [P] Agregar `PLATFORM_JWT_SECRET` y `PLATFORM_DATABASE_URL` a `server/internal/config/`,
...
- [X] T008 [P] Test de `config.Validate` en `server/internal/config/config_test.go` para los tres
      rechazos, incluido el de secretos iguales. Verlos en rojo.
- [X] T009 Abrir el **tercer pool** en `server/cmd/api/main.go` con `PLATFORM_DATABASE_URL`, junto a
      los dos que ya existen.
- [X] T010 `assertPlatformGrantsEnforced` en `main.go`, gemela de la `assertRLSEnforced` que ya
...
- [X] T011 Test de integración de T010: con el rol correcto arranca; con el owner **no**.

### Las dos identidades

- [X] T012 [P] `server/internal/domain/plataforma.go`: qué es un operador y sus sentinels. Puro,
      sin I/O.
- [X] T013 [P] Tests de dominio en `plataforma_test.go`, en tabla.
- [X] T014 Segundo `auth.Manager` firmado con `PLATFORM_JWT_SECRET`.
- [X] T015 **El test que justifica todo el diseño**, en `server/internal/auth/`: un token emitido
...
- [X] T016 [US1] Tests de integración en
...
- [X] T017 [US1] `queries/platform.sql` con las consultas del operador, y `make sqlc`.
- [X] T018 [US1] `app.PlatformService`: autenticar operador, rechazar inactivo.
- [X] T019 [US1] `POST /api/v1/platform/auth/login` en `httpapi/handlers_plataforma.go`, con el
      grupo `/platform` y su middleware propio en `router.go`.
- [X] T020 [US1] Limitador por IP y por cuenta en ese login, reusando el que ya existe.
- [X] T021 [US1] Evento de seguridad en éxito y en fallo (FR-012), con el usuario intentado y
      **nunca** la contraseña.
- [X] T022 [US1] Test de que un operador inactivo se rechaza **igual que uno inexistente**: misma
      respuesta y misma latencia.
- [X] T023 [US1] Test de FR-003: una credencial de plataforma en el login del negocio responde como
...
- [X] T024 [US1] Bandera `-reset-platform-operator` en el binario para crear al primero desde
...
- [X] T025 [US1] Test de que desactivar al operador le corta el acceso **sin esperar** a que caduque
      la sesión (FR-013).
...
- [X] T026 [US3] Test de integración **bajo el rol `gatobobah_platform`**: `select` sobre `orders`,
      `order_payments`, `register_sessions`, `expenses` y `users` da `42501`. Y sobre `companies`,
      un número.
- [X] T026b [US3] Y que **tampoco puede escribir** (FR-009): `insert`, `update` y `delete` sobre
      `companies` dan `42501`.
...
- [X] T027 [US3] Test de la simetría: `gatobobah_app` no puede leer `platform_operators`.
- [X] T028 [US3] Verificar cada test de T026 **quitando el grant correspondiente** y viéndolo pasar
...
- [X] T029 [US2] Test de integración de `GET /api/v1/platform/companies`: lista **todas** las
...
- [X] T030 [US2] Implementar el endpoint.
- [X] T031 [US2] Test de cero empresas: la respuesta lo dice y la pantalla no pinta una tabla vacía
      (FR-015).
...
- [X] T032 [US4] Regla en `web/eslint.config.js`, dentro del bloque `FRONTERAS` que ya existe:
      nadie importa `**/consola/**` desde fuera de `src/consola/`.
- [X] T033 [US4] Verificar que **muerde**: agregar un import cruzado de prueba y ver `bun run lint`
      en rojo. Sin esto, la única barrera contra contaminar la tableta podría estar mal escrita y
      nadie se enteraría.
- [X] T034 [US4] Separar los `tsconfig`: el del POS excluye `src/consola`, y la consola tiene el
      suyo.
...
- [X] T035 [US4] `web/vite.consola.config.ts` con entrada propia, **`outDir: 'dist-consola'`** y
      **`publicDir` propio**.
...
- [X] T036 [US4] La pantalla de la consola en `web/src/consola/`: login y lista de empresas. Sin
      dependencias nuevas.
- [X] T037 [US4] Medir el paquete del POS y compararlo con el **antes**: chunk principal 1,133.50 kB
...
- [X] T037b **(manual, fuera del repositorio)** Tocar el `deploy/.env` de la VM de pruebas y el de
      ...
      2. `PLATFORM_DB_PASSWORD` — `openssl rand -hex 24`.
      3. `CORS_ORIGIN` — agregarle el subdominio de la consola, separado por coma
         (`https://app-dev.elgatobobah.com,https://staff-dev.elgatobobah.com` en pruebas; los de
         producción en producción).

      **Bloquea el despliegue y hay que hacerlo ANTES**: sin las dos primeras la API no arranca
      —fail-fast a propósito— así que un deploy con el código nuevo y sin ellas deja el servicio
      abajo. Sin la tercera la API sí arranca, pero el navegador le bloquea a la consola **todas**
      las llamadas y el síntoma es un error de red que no menciona CORS por ningún lado.

      Los tres valores son aleatorios o mecánicos; ninguno es un dato que alguien tenga que
      recordar. Y antes de editar, respaldar el archivo fuera del repositorio (que es público).

- [X] T038 Agregar a `ci.yml` el build y el despliegue de la consola a sus dos proyectos de Pages,
      **sin tocar** los pasos del POS.
- [X] T039 [P] Documentar el ambiente nuevo en `AGENTS.md` (subdominios, variables) y el control en
      `docs/security-owasp.md`, que es el respaldo del principio V.
- [X] T040 [P] Renglones en `docs/matriz-de-pantallas.md`: qué cubre cada test de aislamiento y **qué
      no**.
- [X] T041 Correr [quickstart.md](./quickstart.md) completo, incluidos los pasos que exigen ver una
      barrera fallar.
- [X] T042 Gates completos: `make api-build`, `api-test`, integración, `make lint`, `make vuln`,
