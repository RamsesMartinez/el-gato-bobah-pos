...
func TestLaCajaNoCierraConPedidosSinTerminar(t *testing.T) {
	...
	// Abierta: no cierra.
	if _, err := backoffice.CloseSession(ctx, principal, cajero, cierreAMano(declarado)); !errors.Is(err, domain.ErrOpenOrders) {
	...
	_, err = backoffice.CloseSession(ctx, principal, cajero, cierreAMano(declarado))
	msg := err.Error()
	if !contiene(msg, "#") {
	...
	if _, err := backoffice.CloseSession(ctx, principal, cajero, cierreAMano(declarado)); err != nil {
...
func TestElArqueoDiceLoQueSeEntregoSinCobrar(t *testing.T) {
	...
	declarado := map[int]decimal.Decimal{int(efectivo): decimal.RequireFromString("50")}
	cerrado, err := backoffice.CloseSession(ctx, principal, cajero, cierreAMano(declarado))
	if err != nil {
